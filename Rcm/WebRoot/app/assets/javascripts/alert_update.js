...
function getResponse(id){
	 ...
	 PostJSONQuery(Url,query_json,function(response){
		 console.info("********user2获得信息详细信息  ="+JSON.stringify(response)+"*******************");
...
//为通知单从数据库查询记录添加到页面    动态追加记录 并使用查询条件进行查询----------计划修-------------//
/**维修通知单 计划修  查询按钮*/
function selectNotifyData_Plan(val8){	
...
/**上一页*/
function backpage_maintence_plan(){
	var nowpage= $('#notify_page_nowpage')[0].innerHTML;
	if(nowpage<=1){
		alert("这是第一页，无法往前");
	}else {
		var backPageNo=nowpage-1;
		selectNotifyData_Plan(backPageNo);
	}
}
/**下一页*/
function nextpage_maintence_plan(){
	var nowpage= $('#notify_page_nowpage')[0].innerHTML;
	...
	if(nowpage>=totalpage){
		...
		selectNotifyData_Plan(nextPageNo);
	}
}
/****************************计划修***************************************************/
var recordCount=1;//global
  ...
  var Url="/Rcm/Manage/getPlanInformList.do";
  PostJSONQuery(Url,notify_query_json,function(response){
	 //alert(JSON.stringify(response));
	  ...
	  var informList=response.planinformList;
	 $('#query_result_table_body tr').remove();
	  for(var i=0;i<informList.length;i++){
		  ...
		  $('#query_result_table_body').append("" +
		  		...
		  		"<td scope='col' >"+informList[i]["maintain_id"]+"</td>" +
		  		...
		  		"<td scope='col' >"+informList[i]["warningTypeLevel"]+"</td>" +
		  		"<td scope='col' >"+
		  		"<span   onmouseover='showDetail(true,this);' onMouseOut='showDetail(false,this);'>"+informList[i]["maintaininfo"].substring(0,3)+"..."+"</span>"+
		  		"<div class='detail_info'>"+
           			"<textarea style='height: 150px; width:250px;'rows='6' cols='1'>"+informList[i]["maintaininfo"]+
           			"</textarea>"+
        		"</div>"+
		  		"</td>" +
		  		"<td scope='col' >"+informList[i]["maintain_period"]+"</td>" +
		  		"<td scope='col' >"+informList[i]["next_maintainDate"]+"</td>" +
		  		"</tr>");
	  } 
    });  
}
	...
	//-----------------------------故障告警维保通知单--------------------------------//
	function OnNotify_faultAlertQuery(query_notify){
	...
	/**维修通知单  故障告警  查询按钮*/
function selectNotifyData(val8){	
	var val1 = $('#condition1').val();
	var val2 = $('#condition2').val();
	var val3 = $('#condition3').val();
	var val4 = $('#condition4').val();
	...
	OnNotify_faultAlertQuery(query_notify);
}
...
/**通知单维保反馈记录--------------------------------------------------------------------------*/
  ...
  PostJSONQuery(Url,notify_query_json,function(response){
	...
	/**查询按钮*/
function selectMaintenceData(val7){	
	  ...
	  PostJSONQuery(Url,alert_query_json,function(response){
		...
		$("#query_result_table_body tr").remove();
		  for(var i=0;i<planMaintenceAlerts.length;i++){
			  //alert(JSON.stringify(planMaintenceAlerts[i].statments));
			//  alert(planMaintenceAlerts[i]['id']+","+planMaintenceAlerts[i]['equipmentId']);  ok
			var onmouse=null;
				onmouse="<tr style='height: 26px; >";
				//将未处理的告警信息显示出来
				var number=((( $('#main_page_nowpage')[0].innerHTML-1)*10)+1+i);
				 $('#query_result_table_body').append("" +
					  onmouse +
			  		"<th></th>" +
			  		"<td  scope='col' ></td>" +
					"<td  scope='col' id='user1_emerge1_id'>"+number+"</td>" +
			  		...
			  		"<td scope='col' >"+
			  		"<span   onmouseover='showDetail_text(true,this);' onmouseout='showDetail_text(false,this);'>"+planMaintenceAlerts[i]["warninginfo"].substring(0,5)+"..."+"</span>"+
			  		"<div class='detail_info'>"+
...
function backpage_app_plan(){
	...
	if(nowpage<=1){
		...
		var backPageNo=nowpage-1;
		 $('#main_page_nowpage').html(backPageNo);
		selectData_plan(backPageNo);
	}
	
	
}
/**下一页*/
function nextpage_app_plan(){
	$("#select_all").attr('checked',false);
	var nowpage= $('#main_page_nowpage')[0].innerHTML;
	...
	if(nowpage>=totalpage){
		...
		var Page=nowpage-1;
		var nextPageNo=Page+2;
		 $('#main_page_nowpage').html(nextPageNo);
		selectData_plan(nextPageNo);
	}
}
/**查询按钮*/
function selectData_plan(val0){	
