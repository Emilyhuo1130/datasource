 ...
 $.createDialog=function(msg,callback){
	 var dialog_default={
		...
		$document.append("<div id='alertDialog' style='width:500px;height:100px;'>" +
				"<div style='height:20px;background-color:#0000e1;'><span style='color:#fff;font-size;.7em;'>消息提示</span></div>" +
				"<div style='height:80px;background-color:#fff;'>" +
				"<div><div style='height:10px;'></div><span  id='msg' style='width:300px;'>"+"&nbsp;&nbsp;&nbsp;"+msg+"</span></div>"+
					"<div style='position:relative;left:50%;top:30%;'>" +
						"<button id='btn_ensure' 	style='width:70px;height:20px;  cursor:pointer;'>确认</button>" +
						"<button  id='btn_cancel'  style='position:relative;left:50px;width:70px;height:20px; cursor:pointer;'>取消</button>" +
					"</div>"+
				"</div>" +
				"</div>");
	//如果文字过长自动截取一段
		if($("span[id='msg']").html().length>60){
			$("span[id='msg']").html($("span[id='msg']").html().substring(0,60)+"...");
